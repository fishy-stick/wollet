...
internal sealed class CompatibilityHint : FlowLayoutPanel
{
    ...
    {
        var version = "服务端版本：" + (result?.ServerVersion ?? "待确认") + (result?.Historical == true ? "（上次连接）" : "");
        var badge = result?.Label ?? "待确认";
        var details = result is null ? "连接后可确认版本和功能支持。" :
        ...
        if (result is not null && FeatureCatalog.Normalize(result.ServerVersion) == "")
            details += "\n\n服务端未提供有效版本号。旧版服务端或未设置 VERSION 的本地构建可能出现此情况；刷新不会生成版本号。";
        // A periodic refresh with unchanged data must not dismiss an open hint.
        if (_version.Text == version && _badge.Text == badge && _details.Text == details) return;
        _popup.Close();
