...
状态：设计草案，尚未实现。日期：2026-09-19。

本文定义客户端主导执行的关机计划，供服务端、管理页面和 Windows 客户端共同实现。现行协议见 [protocol.md](../protocol.md)，版本范围见 [开发计划](../development-plan.md)。
...
`v1.1.0` 实现固定 10 秒倒计时、管理页面取消与立即执行、本地取消、状态同步及旧版兼容。动态调整时长、自定义长时预约和休眠不属于本版本交付范围。
...
| 客户端桌面提示进程 | 展示后台服务提供的剩余时间，将本地取消操作交给后台服务 |
...
客户端按计划串行处理网络指令、本地取消及计时到期。服务端不能自行增加客户端的 `revision`。收到较旧快照时可以结束对应请求的等待，但不得用其覆盖较新的状态。
...
| 桌面提示进程退出或暂时不可用 | 后台计划继续，报告提示不可见；不等同于后台服务重启 |
...
| 关闭提示窗口 | 仅隐藏提示，计划继续；窗口需要明确区分“隐藏”和“取消关机” |
...
本地提示进程通过受访问控制的 IPC 向后台服务请求取消，不读取设备密钥，也不直接获得关机权限。后台服务验证调用会话、计划 ID 和当前状态。IPC 传输与窗口形式单独设计。
